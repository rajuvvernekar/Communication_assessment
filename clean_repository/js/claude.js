...
Score 5: Agent greeted with "Good Morning/Good Afternoon/Good Evening, Thank you for calling Broker, my name is [Name], how may I assist you today?" — all four elements present.
...
Return ONLY this JSON: {"pass": true} or {"pass": false}`;
  ...
  async function callAiCustomer(scenario, description, messages, turnNumber, maxTurns) {
    ...
    const system = `You are roleplaying as a frustrated but articulate customer calling Broker's customer support line.

SCENARIO: ${scenario || description || 'A client has an issue with their trading account.'}

YOUR PERSONA:
- Long-term Broker investor who is knowledgeable about trading
