  ...
  async function _seedManagerTopicsImpl() {
    try {
      ...
      const mgrTopics = [
        ...
        // ── NRI Manager (2026-09-25, from NRI_Team_Sales_Manager_Assessments.docx) ──
        // Their own module keys (never in REPLACED_MGR_MODULES below), so admin-created
        // NRI topics are never pruned. Live-read from this table by the manager portal.
        { id: "mgr-nri-sr1", module: "mgr-nri-situation-room", title: "The Vanishing Application", enabled: true, description: "The Vanishing Application", scenario: "An NRI client based in Dubai has been trying to upload her pending KYC documents and submit her account-opening application through the app for the past four days. Every attempt fails silently — no error message, no confirmation, nothing. She has already been through standard troubleshooting twice with the support desk: clearing cache, switching browsers, re-uploading the documents in a different file size. Nothing has worked. She has already paid the account-opening fee upfront, and she is now calling you directly, asking pointedly whether this account is “ever going to open”, and whether she has simply lost her money to a broken system.\n\nPart A — What Would You Say? Write your full verbal response, opening to close.\nPart B — The Wrong Response: A flawed manager reply to this situation follows below. Identify every error, explain the impact of each, and rewrite the response correctly.\n\n─── THE WRONG RESPONSE (given to the manager to critique) ───\n\n\"Ma'am, I understand this is frustrating, but the upload issue isn't something I can see from my end. You've already cleared your cache and switched browsers twice with the support desk, so I'd suggest trying once more, maybe from a different device. The fee you paid is the standard account-opening charge, so I can't really say anything about it at this stage. Please keep trying for a few more days, and if it still doesn't work, just raise another ticket with the support desk.\"\n\n─── INTERNAL DATA ───\nClient: NRI based in Dubai, account-opening application in progress; the account-opening fee was paid upfront and is reconciled against her application\nUpload history: Four days of failed KYC document uploads that fail silently, with no error shown to the client\nAlready tried: Cache cleared, browser switched and documents re-uploaded in a different file size, twice with the support desk\nWhat the logs show: Nothing on the client side; the backend upload logs for this user need an engineering check (file size, format and document-type validation)\nFee status: The fee stays with her application and is not lost; a refund is possible if the account cannot be opened, subject to NRI operations approval\nFallback to confirm: Assisted submission of the KYC documents through the NRI onboarding desk if the app upload cannot be fixed quickly\nOwnership: Raise a priority engineering ticket, name an NRI operations owner, and commit to an update within 1 working day", checklist: [] },
        { id: "mgr-nri-ta1", module: "mgr-nri-transcript-autopsy", title: "The Rejected Courier", enabled: true, description: "The Rejected Courier", scenario: "BACKGROUND: An NRI client's overseas address proof was in her husband's name, not her own — a mismatch that should have been caught during document review before the papers were approved for courier. The hard copies were shipped at the client's cost and, on arrival, rejected for exactly this reason. The client, who has already paid for and completed one courier, is now on the line asking what to do next. She has proposed two of her own alternatives: send the utility bill together with her marriage certificate as supporting proof, or send a six-month UAE salary account statement that is in her own name.\n\n─── CALL TRANSCRIPT ─────────────────────────────────────────────\n\nCLIENT: \"Hello, I'm calling about my NRI account. I sent my address proof for courier — the utility bill — and I've just been told it's been rejected because it's not in my name, it's in my husband's name. I don't understand why this wasn't caught before I paid to courier it.\"\n\nMANAGER: \"Ma'am, the courier team can only process what's already been approved by document verification. If the name doesn't match, it gets flagged and rejected at the final stage.\"\n\nCLIENT: \"But someone on your side approved it for courier in the first place. Why does the mismatch only get caught after I've already spent money sending it?\"\n\nMANAGER: \"I can't really speak to that, ma'am, different teams handle different stages. What I can tell you is that the name on the address proof has to match the applicant.\"\n\nCLIENT: \"I understand that now. I have a marriage certificate — can I send that along with the utility bill to establish the connection? Or should I send my UAE salary account statement instead, which is in my own name?\"\n\nMANAGER: \"Either one might work, I'm not entirely sure. You could try sending both and see which one gets accepted.\"\n\nCLIENT: \"I don't want to “try” again. I already paid for one courier that got rejected because of a mistake on your side. I need to know for certain before I send anything else.\"\n\nMANAGER: \"I understand your frustration, but I can't guarantee approval until our verification team actually reviews it.\"\n\nCLIENT: \"So you're asking me to spend money on a second courier with no certainty at all?\"\n\nMANAGER: \"Ma'am, that is the process. I can raise a query with the verification team, but I can't promise a timeline.\"\n\nCLIENT: \"This is unacceptable. Nobody has even apologised for the fact that this should never have been approved for courier the first time. I want this in writing, and I want to speak to your senior.\"\n\nMANAGER: \"I'll note your feedback, ma'am. Someone will get back to you.\"\n\n─── INTERNAL DATA ───\nClient: NRI in the UAE, applicant is the wife; the overseas address proof (a utility bill) is in her husband's name\nCourier 1: Shipped at the client's cost and rejected at the final stage for the name mismatch; the mismatch should have been caught at document review before the courier was approved\nDocument rule: The overseas address proof must be in the applicant's own name; a document in another person's name is not acceptable on its own\nOptions the client proposed: (1) the utility bill plus a marriage certificate to link the names, or (2) a six-month UAE salary account statement in her own name\nCertainty before courier 2: The verification team can pre-check scanned copies before another courier is sent, so the client does not pay again on a guess\nYour authority: You cannot approve a document yourself, but you can get the verification team's written decision before the client sends anything\nOwnership: The earlier miss is the firm's; acknowledge it, apologise and give a written follow-up with a named owner", checklist: [] },
        { id: "mgr-nri-mc1", module: "mgr-nri-mock-call", title: "Partner Branch Document Failure — NRI Address Proof Rejection", enabled: true, description: "Partner Branch Document Failure — NRI Address Proof Rejection", scenario: "Mr. Rakesh Menon, an NRI client based in Doha, submitted his NRI account-opening forms — including his overseas address proof — through Zerodha's partner branch in Meerut on 14th August 2026. The branch accepted the physical documents and forwarded them for processing.\n\nUnder Zerodha's KYC norms, an overseas address proof — even one that is already in English — must be accompanied by a notarised translation or an apostille/embassy attestation valid for the client's country of residence. A document in English alone, without that additional certification, does not meet the requirement. The partner branch did not check for this before accepting Mr. Menon's documents.\n\nOn 2nd September 2026, nearly three weeks later, the client was informed that his hard-copy submission had been rejected — the overseas address proof was in English only, without the required notarisation, and was therefore invalid. The client is now on an escalation call. He wants to know exactly what went wrong, why it wasn't caught earlier, and what needs to be resent. He is also asking pointed questions about whether this is a recurring problem at that branch and what is being done about it. The manager must handle this call with complete ownership of the branch's miss, precise documentation knowledge, and a clear, confident path forward — without becoming defensive or shifting blame onto the client.\n\nThe client opens the call by saying:\n\n\"I submitted every document your Meerut branch asked me for, on the date they told me to. Three weeks later, I'm told it's all been rejected. I want to know exactly what went wrong, and why it's taken this long for anyone to tell me.\"\n\nEscalation beats the client will raise if your handling doesn't already address them: (1) \"You're telling me my address proof — which is entirely in English, issued by my bank in Doha — wasn't accepted because it's in English? That doesn't make sense to me. Explain that properly, because right now it sounds like an excuse.\" (2) \"So if I understand you correctly, your own branch staff accepted my documents without checking whether they met your own requirements, and I am the one who has lost three weeks because of it. How does that happen at a Zerodha partner branch?\" (3) \"I do business across three countries, and I have never had this much trouble opening a demat account anywhere. If Zerodha's KYC process is this unreliable, I want to know whether this is something I should be raising with SEBI or your grievance redressal desk, because right now this feels like negligence, not a genuine documentation issue.\" (4) \"I don't want to be told ‘documents’ in general. Tell me exactly, in writing, what you need from me this time — the KYC page, the address proof, whatever it is — because I am not going through another three-week cycle over an ambiguous checklist.\" (5) \"Before I agree to send anything again, I want to know one more thing. What is Zerodha actually doing about the fact that your own partner branch let this through? Because if nothing changes there, the next NRI client who walks into that branch is going to have exactly the same problem I did.\"\n\nHandle this call for 5-6 minutes: acknowledge the issue and the client's frustration without over-explaining excuses, take clear ownership of what your organization controls, respond to each escalation beat as it comes up, and close with a resolution that is realistic and within your actual authority.\n\n─── INTERNAL DATA ───\nClient: Mr. Rakesh Menon, based in Doha; NRI account-opening forms and overseas address proof submitted through the Meerut partner branch on 14 August 2026\nRejection: Informed on 2 September 2026, 19 days after submission; the overseas address proof was in English only, without the required attestation\nKYC rule: An overseas address proof, even in English, must be accompanied by a notarised translation or an apostille or embassy attestation valid for the country of residence; English alone does not meet it\nWhere it went wrong: The Meerut partner branch accepted and forwarded the documents without checking for the attestation\nWhat the client must resend: The address proof with a valid attestation or notarised translation, plus the KYC page; send the exact checklist in writing after the call\nGrievance route: The client may escalate to the firm's grievance redressal desk and to SEBI SCORES; give the contact details if asked\nSystemic fix: Confirm the current status of the partner-branch review with the Partner Branch Quality team before you commit to any specific corrective action", checklist: [] },

        // ── Trainer demo topics (one per assessment; title prefix "[DEMO]"). TA and
        // Paper Trade demos are live-read; Situation Room's demo is read from here
        // too (regular SR stays hardcoded); Red Pen / Mirror Room demos are
        // hardcoded in manager-app.js and these two rows are admin-listing only.
        { id: "mgr-demo-sr", module: "mgr-situation-room", title: "[DEMO] Walkthrough — Delayed Fund Credit", enabled: true, description: "[DEMO] Walkthrough — Delayed Fund Credit", scenario: "A regular client initiated a ₹50,000 fund transfer this morning. The bank confirms the debit, but two hours later the money is still not showing in the trading balance, and the client wants to place a trade before the market moves. The client has called you directly, worried and a little impatient. This is a trainer walk-through topic: it is deliberately low-stakes so the flow of the assessment is the focus.\n\nPart A — What Would You Say? Write your full verbal response, opening to close.\nPart B — The Wrong Response: A flawed manager reply to this situation follows below. Identify every error, explain the impact of each, and rewrite the response correctly.\n\n─── THE WRONG RESPONSE (given to the manager to critique) ───\n\n\"Sir, fund transfers sometimes take time, there's nothing I can do about it from here. Just wait a while and it will show up. If it doesn't, raise a ticket on the support portal and someone will get back to you.\"\n\n─── INTERNAL DATA ───\nClient profile: Regular client for 3 years, no prior complaints\nIssue: A ₹50,000 fund transfer initiated this morning is not yet showing in the trading balance\nPayment status: The bank confirms the debit; the payment gateway shows the credit as pending reconciliation\nNormal timeline: Credits normally reflect within 30 minutes; this one is at 2 hours\nYour authority: You can raise a priority payments ticket and give the client the ticket number; you cannot credit funds manually\nNext update: Payments operations replies within 2 working hours on priority tickets", checklist: [] },
        { id: "mgr-demo-ta", module: "mgr-transcript-autopsy", title: "[DEMO] Walkthrough — The Delayed Credit", enabled: true, description: "[DEMO] Walkthrough — The Delayed Credit", scenario: "BACKGROUND: A client's ₹25,000 fund transfer sent yesterday evening has only just reflected in the trading balance this morning, and they call to ask why it took so long. This is a trainer walk-through topic: short and low-stakes so the flow of the assessment is the focus.\n\n─── CALL TRANSCRIPT ─────────────────────────────────────────────\n\nCLIENT: \"I sent ₹25,000 yesterday evening and it only showed up this morning. Why did it take so long?\"\n\nMANAGER: \"Sir, transfers can take time. It's showing now, so there's nothing to worry about.\"\n\nCLIENT: \"But I needed that money yesterday. Can you tell me what actually caused the delay?\"\n\nMANAGER: \"I don't know exactly, sir. It could be the bank or it could be us.\"\n\nCLIENT: \"That's not much of an answer. How do I make sure this doesn't happen again?\"\n\nMANAGER: \"Just send it earlier next time, sir. Anything else?\"\n\n─── INTERNAL DATA ───\nClient profile: Client for 2 years, first complaint about a delayed credit\nTransfer: ₹25,000 sent from the client's bank yesterday at 4:10 PM, after the bank's cut-off for same-day credit\nPayment status: Received by the firm at 9:05 AM today and credited at 9:20 AM\nExplanation: The delay came from the bank cut-off, not from the firm; the funds are now available\nFirm guideline: Explain the cut-off timing plainly, confirm the funds are available now, and share how to avoid it next time\nEscalation: None needed unless the client reports the funds are still missing", checklist: [] },
        { id: "mgr-demo-mc", module: "mgr-mock-call", title: "[DEMO] Walkthrough — Delayed Withdrawal", enabled: true, description: "[DEMO] Walkthrough — Delayed Withdrawal", scenario: "A client requested a ₹30,000 withdrawal yesterday evening and is calling because the money has not reached their bank account this morning. This is a trainer walk-through topic: a short, low-stakes call so the flow of the assessment is the focus.\n\nThe client opens the call by saying:\n\n\"I requested a withdrawal of ₹30,000 yesterday evening and it's still not in my bank account. What's going on?\"\n\nEscalation beats the client will raise if your handling doesn't already address them: (1) \"Your website said withdrawals are processed quickly. Why hasn't mine arrived yet?\" (2) \"I need this money today. Can you at least tell me exactly when it will reach me?\"\n\nHandle this call for 5-6 minutes: acknowledge the issue and the client's frustration without over-explaining excuses, take clear ownership of what your organization controls, respond to each escalation beat as it comes up, and close with a resolution that is realistic and within your actual authority.\n\n─── INTERNAL DATA ───\nClient profile: Client for 18 months, one earlier withdrawal completed without any issue\nWithdrawal: ₹30,000 requested yesterday at 6:40 PM, after the payout cut-off\nPayout status: Queued for the next payout run at 11:00 AM today; the bank credit follows within 1 hour\nNormal timeline: Requests after 4:00 PM are processed on the next working day\nYour authority: You can confirm the payout time and the reference number; you cannot release funds outside the payout run\nWhat to give the client: The payout reference number and the expected credit time", checklist: [] },
        { id: "mgr-demo-fb", module: "mgr-feedback", title: "[DEMO] Walkthrough — The Repeatedly Late Log-In", enabled: true, description: "[DEMO] Walkthrough — The Repeatedly Late Log-In", scenario: "Employee: Sam — a friendly, well-liked agent with solid QA scores who has logged in 10-15 minutes after shift start on 6 of the last 10 working days. He has not raised a reason.\n\nSituation: You are speaking with Sam privately for the first time about the pattern. This is a trainer walk-through topic: it is intentionally low-stakes so the flow of the assessment, not the difficulty, is the focus.\n\nThink About:\n- How will you open without making Sam feel accused?\n- What specific pattern (dates, times) will you point to?\n- What will you agree with Sam before you end the conversation?", checklist: [] },
        { id: "mgr-demo-eq", module: "mgr-eq", title: "[DEMO] Walkthrough — A Short Workday", enabled: true, description: "[DEMO] Walkthrough — A Short Workday", scenario: "A trainer walk-through case: one short workday told across 3 linked sections, each with its own conversational-AI counterpart and 2 exchanges.\n\n─── SECTION A — The Missed Handover (counterpart: Kavya) ───\nA colleague tells you a handover note you promised yesterday never arrived, and she started her shift without it.\n\n─── SECTION B — The Stalled Project (counterpart: Dev) ───\nDev tells you a two-week project feels pointless now that priorities have changed and he is losing energy for it.\n\n─── SECTION C — The Tense Meeting (counterpart: Isha) ───\nIsha stays back after a meeting, upset that a colleague spoke over her twice in front of everyone.", checklist: [] },
      ...
      const MGR_TA_CONTENT_VERSION = 6; // v6 (2026-09-25): every scenario now ends with an "INTERNAL DATA" section (client-context box shown to the manager) -- this bump is what pushes it to already-seeded databases. v5 (2026-09-24): rewrote the 4 transcripts so each has exactly 5 manager turns -- opening, acknowledgment, empathy, ownership, closing, each its own distinct blunder -- to match the new per-turn correction UI (see manager-app.js's Transcript Autopsy rewrite)
      const MGR_MC_CONTENT_VERSION = 5; // v5 (2026-09-25): every scenario now ends with an "INTERNAL DATA" section (client-context box shown to the manager) -- this bump is what pushes it to already-seeded databases. v4 (2026-09-20): full replace with the exact 4 cases from the manager training doc (Unlisted Share Transfer & Regulatory Intimation, Minor to Major Account Conversion, Kill Switch Malfunction, Outdated App NAV Display)
      const MGR_SR_CONTENT_VERSION = 2; // v2 (2026-09-25): every scenario now ends with an "INTERNAL DATA" section (client-context box shown to the manager) -- this bump is what pushes it to already-seeded databases. v1 (2026-09-20): scenario text now embeds "THE WRONG RESPONSE" (Part B's flawed sample reply) so admin's Topics tab shows it -- it was previously saved only in manager-app.js's hardcoded SCENARIOS constant, invisible anywhere in the admin panel
      const MGR_EQ_CONTENT_VERSION = 2; // v2 (2026-09-24): fixed \\n literally appearing as two characters instead of a line break in the section text (an escaping slip -- \\\\n instead of \\n in this file's own source -- introduced when these rows were first added at v1)
      const MGR_FB_CONTENT_VERSION = 2; // v2 (2026-09-24): same \\n-escaping fix as MGR_EQ_CONTENT_VERSION above, in the 6 Red Pen rows added at v1
      ...
      if (_useLocalStorage) {
        ...
        try {
          ...
          await _refreshOpsScriptIfStale('mgr-eq', MGR_EQ_CONTENT_VERSION, mgrTopics, localT);
          await _refreshOpsScriptIfStale('mgr-feedback', MGR_FB_CONTENT_VERSION, mgrTopics, localT);
        } catch (refreshErr) {
      ...
      try {
        ...
        await _refreshOpsScriptIfStale('mgr-eq', MGR_EQ_CONTENT_VERSION, mgrTopics, existing || []);
        await _refreshOpsScriptIfStale('mgr-feedback', MGR_FB_CONTENT_VERSION, mgrTopics, existing || []);
      } catch (refreshErr) {
