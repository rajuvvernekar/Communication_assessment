...
window.Admin = (() => {
  ...
  async function loadTopics() {
    ...
    try {
      ...
      if (allTopics.filter(t => t.module === 'stock-market-mcq' && t.title && (t.title.includes('Set 1') || t.title.includes('Set 2'))).length < 2) {
  ...
  async function seedStockMarketMcq(silent = false) {
    ...
    // Delete legacy topics that predate the Set 1 / Set 2 naming
    const oldTopics = smqTopics.filter(t => !t.title || (!t.title.includes('Set 1') && !t.title.includes('Set 2')));
    for (const old of oldTopics) {
      try { await DB.del('topics', old.id); } catch (_) {}
    }

    const validTopics = smqTopics.filter(t => t.title && (t.title.includes('Set 1') || t.title.includes('Set 2')));
