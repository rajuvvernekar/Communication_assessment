...
for target in admin_files:
    if os.path.exists(target):
        ...
        if "seedManagerTopics" not in code:
            code = code.replace("async function loadTopics() {", manager_seed_code + "\n  async function loadTopics() {")
            code = code.replace("renderTopicsList();", "await seedManagerTopics();\n    renderTopicsList();")
            with open(target, "w", encoding="utf-8") as f:
                f.write(code)
            print(f"Added seedManagerTopics to {target}")
