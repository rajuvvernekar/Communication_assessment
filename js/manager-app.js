'use strict';
// ============================================================
//  CommAssess — Manager Portal App  v1
//  Handles auth, module navigation, audio/written/MCQ flows
// ============================================================
  ...
  const SCENARIOS = {
    ...
    'mgr-feedback': [
      { id:'fb1', title:'The Burnout Star',
        scenario:'Context: Rahul is your best performer — always exceeds targets. For the last 3 weeks he has been arriving late, missing standups, and giving short, impatient responses to teammates. His output has dropped 20%.\n\nYou have called Rahul for a one-on-one. Give him structured, empathetic feedback and uncover what is happening. You have 4-5 minutes.' },
      { id:'fb2', title:'The Struggling New Hire',
        scenario:'Context: Priya joined 8 weeks ago. She is technically capable but struggles with customer-facing communication — often comes across as too blunt. Two customers have complained. Her teammates are starting to pick up the slack.\n\nConduct a supportive performance conversation with Priya. The goal is to be honest about the impact while keeping her engaged and confident. You have 4-5 minutes.' },
      { id:'fb3', title:'The Dismissive Senior',
        scenario:'Context: Arjun has 5 years of experience and is technically your best agent. However, he consistently dismisses new processes, is condescending to newer team members in public, and says things like "we have always done it this way."\n\nGive Arjun clear, direct feedback on his behaviour and its impact on team culture. You have 4-5 minutes.' },
    ],
    'mgr-eq': [
...
— Priya"`;
  ...
  const MODULE_META = {
    ...
    'mgr-feedback':          { label: 'Feedback',              type: 'audio',   icon: '💬' },
  ...
  // ── Internal state ───────────────────────────────────────
  let _currentModule   = null;  // moduleKey string
  let _currentScenario = null;  // scenario object
  let _recordingBlob   = null;
  let _recordingPromise = null; // Promise<blob> from Recorder.start()
  let _transcript      = '';
  let _prepTimer       = null;
  let _recStartTime    = null;
  let _mcqAnswers      = [];    // selected option index per question
  ...
  function scoreWrittenResponse(text, moduleKey) {
    ...
    return {
      contentScore, clarityScore, empathyScore, actionScore, criticalThinkingScore,
      overall, wordCount: words,
      _method: 'mgr-written', _module: moduleKey,
    };
  }
  ...
  async function login() {
    ...
    try {
      ...
      let user;
      try {
        user = await Auth.signIn(empId, password);
      } catch (signInErr) {
        // New user — sign up
      ...
      btn.disabled = false;
      btn.textContent = 'Enter Manager Portal →';
    }
  }
  ...
  async function startRecording() {
    ...
    // Start waveform
    const canvas = $('mgr-waveform');
    Recorder.startWaveform(canvas);

    // Start timer (count-up, max 5 min = 300s)
    _recStartTime = Date.now();
    const timerEl = $('mgr-rec-time');
    const MAX_REC_SECS = 300;
    Recorder.startTimer(timerEl, MAX_REC_SECS, null, () => {
      // Auto-stop at 5 minutes
      stopRecording();
    }, true);

    // Start transcription (Chrome only — won't error on other browsers)
    _transcript = '';
    try {
    ...
    } catch (e) { /* Not Chrome — silently continue */ }

    // Start actual recording — save promise so stopRecording can await the blob
  ...
  async function stopRecording() {
    ...
    // Stop recorder — this triggers onstop which resolves _recordingPromise
    ...
    await submitAudio(finalTranscript, durationSecs, blob);
  }

  async function submitAudio(transcript, durationSecs, blob) {
    try {
      const analysis = SpeechEngine.analyze(transcript, durationSecs);
      const aiScores = SpeechEngine.scoreSpeech(analysis, durationSecs);
  ...
  async function submitMCQ() {
    ...
    if (unanswered > 0) {
      toast(`Please answer all questions. ${unanswered} question(s) remaining.`, 'error');
      return;
    }
  ...
  function _showResult(aiScores, type) {
    ...
    if (type === 'mcq') {
      $('mgr-result-subtitle').textContent =
        `Listening & Tone — ${aiScores.correct}/${aiScores.total} correct`;
      $('mgr-result-score').textContent = `${aiScores.overall}%`;
      $('mgr-score-grid').innerHTML = `
        <div class="mgr-score-item">
          <div class="label">Correct Answers</div>
          <div class="val">${aiScores.correct} / ${aiScores.total}</div>
        </div>
        <div class="mgr-score-item">
          <div class="label">Score</div>
          <div class="val">${aiScores.overall}%</div>
        </div>`;
      ...
      const rows = [
        ...
        ['Time Management', aiScores.timeManagement],
  ...
  function backToModules() {
    _clearPrepTimer();
    try { Recorder.stop(); } catch (e) { /* ignore if not recording */ }
    try { Recorder.stopTimer(); } catch (e) { /* ignore */ }
    try { SpeechEngine.stopTranscription(); } catch (e) { /* ignore */ }
    _recordingPromise = null;
    showScreen('mgr-screen-modules');
  }
  ...
  function _bindEvents() {
    ...
    const btnStopRecord = $('btn-mgr-stop-record');
